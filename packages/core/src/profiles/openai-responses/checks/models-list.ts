...
export const openaiResponsesModelsListCheck: WireCheck = {
  ...CHECK,
      ...
      return checkResult({
        ...
        message:
          missingRecommendedFields.length > 0
            ? `Models list response is valid, but recommended fields are missing: ${missingRecommendedFields.join(", ")}.`
            : "Models list response is valid.",
        details,
        startedAt,
      });
    } catch (error) {
...
function collectMissingRecommendedFields(
  ...
  if (typeof body.object !== "string") missing.add("object");
  ...
  for (const item of data) {
    ...
    if (typeof item.object !== "string") missing.add("item.object");
    if (typeof item.created !== "number") missing.add("item.created");
    if (typeof item.owned_by !== "string") missing.add("item.owned_by");
  }
